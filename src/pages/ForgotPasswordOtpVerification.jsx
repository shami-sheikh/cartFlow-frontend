...
import { FiArrowLeft, FiClock, FiMail, FiCheckCircle } from 'react-icons/fi';
import { Loader } from 'lucide-react';
import { Link, useNavigate, useLocation } from 'react-router-dom';
import { useDispatch, useSelector } from 'react-redux';
import { toast } from 'sonner';
import {verifyForgotPassword,resendForgotPasswordOtp } from '../redux/slices/authSlice.js';
...
const ForgotPasswordOtpVerification = () => {
  ...
  useEffect(() => {
    const otpExpire = localStorage.getItem('passwordOtpExpire');
    const resendExpire = localStorage.getItem('passwordResendExpire');

    if (otpExpire) {
      const remaining = Math.floor((parseInt(otpExpire) - Date.now()) / 1000);
      setExpireTime(remaining > 0 ? remaining : 0);
    } else {
      setExpireTime(300); // 5 minutes for password reset
      localStorage.setItem('passwordOtpExpire', Date.now() + 300000);
    }
  ...
  const handleResendOTP = async () => {
    try {
      ...
      const res = await dispatch(resendForgotPasswordOtp(otpData)).unwrap();

      // Update timers
      const newExpire = Date.now() + 600000; // 5 minutes
      localStorage.setItem('passwordOtpExpire', newExpire);
      setExpireTime(300);

      const newResendExpire = Date.now() + 15000; // 15 seconds
      localStorage.setItem('passwordResendExpire', newResendExpire);
      setResendTimer(15);

      toast.success(res?.toast?.text || res?.message || "OTP resent successfully!", { duration: 2000 });
    } catch (err) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await dispatch(verifyForgotPassword({ 
        email: emailFromState, 
        otp 
      })).unwrap();

      toast.success(response?.message || "OTP verified successfully!", { duration: 2000 });
      setOtpVerified(true);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 to-black flex items-center justify-center p-6">
        <div className="max-w-md w-full">
          <div className="bg-gray-800 bg-opacity-50 backdrop-blur-sm border border-gray-700 rounded-2xl p-8 shadow-2xl text-center">
            <div className="w-16 h-16 bg-green-500/10 border border-green-500/20 rounded-full flex items-center justify-center mx-auto mb-6">
              <FiCheckCircle className="text-2xl text-green-400" />
            </div>
            
            <h1 className="text-2xl font-bold text-white mb-4">OTP Verified Successfully!</h1>
            
            <p className="text-gray-300 mb-6">
              Your OTP has been verified. You will be redirected to reset your password shortly.
            </p>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-black flex items-center justify-center p-6">
      <div className="max-w-md w-full">
        <Link to="/forgot-password" className="inline-flex items-center text-yellow-400 hover:text-yellow-300 mb-8 transition-colors">
        ...
        <div className="bg-gray-800 bg-opacity-50 backdrop-blur-sm border border-gray-700 rounded-2xl p-8 shadow-2xl">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-300">Enter the 4-digit code sent to your email for password reset</p>
